...
public interface BaseService<T, ID extends Serializable,  M extends BaseMapper<T, ID>> {
	 ...
	 * @return 统计数据列表
	 */
	default List<Map<String, Object>> statisticByCriteria(@Param(Constants.CRITERIA) StatisticPredicate predicate) {
	 ...
	 * @return 统计数据列表
	 */
	default <E> List<E> statisticByCriteria(@Param(Constants.CRITERIA) StatisticPredicate predicate, @Param(Constants.RESULT_TYPE) Class<E> resultType) {
	 ...
	 * 目前支持Mysql、PostgreSQL、Oracle、DM（达梦）、OpenGauss。或兼容并使用以上数据库驱动的数据库。<br>
	 * 已知问题：<br>
	 ...
	 * @return 影响行数
	 */
	default int batchMerge(List<T> entityList, String conflictIndexName) {
		return getDomainMapper().batchMerge(entityList, conflictIndexName);
	}
}
